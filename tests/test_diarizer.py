...
def test_diarize_audio_missing_token():
    config = AppConfig(diarization=DiarizationConfig(enabled=True, auth_token=""))
...
def test_diarize_audio_success():
    ...
    config = AppConfig(diarization=DiarizationConfig(enabled=True, auth_token="hf_test_token"))
    
    # Mocking the pipeline return
    mock_turn = MagicMock()
    mock_turn.start = 1.0
    mock_turn.end = 2.5
